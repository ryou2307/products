...
Arrays.fill(distance, Integer.MAX_VALUE);// 編集: 課題3 始点からの距離の配列distanceの全要素をInt型で扱える範囲の最大値に初期化: 無限大はInteger.MAX_VALUEを使うと便利
Arrays.fill(done, false);// 編集: 課題3 最短距離確定済頂点集合Xについて，配列doneの全要素を未確定(false)で初期化．
Arrays.fill(vertex, true);// 編集: 課題3 参照用の配列vertexの全て全要素をtrueで初期化
distance[s] = 0;// 編集: 課題3 始点sの距離distanceを0に初期化
// 初期化はここまで
//////////////////////// 課題3. ///////////////////////////////
...
while (!Arrays.equals(done, vertex)) {
  int minD = Integer.MAX_VALUE;
  int minD_Vertex = -1;

  // V-X (X以外の部分) の中で，Dの値が最小の頂点をuとする計算
  for (int i = 0; i < numV; i++) {
      if (!done[i] && distance[i] < minD) {
          minD = distance[i];
          minD_Vertex = i;
      }
  }// 編集: 課題3. while文の完成．doneの中身が全てtrueになり，vertexと等しくなるまで回す．
//////////////////////// 課題3. ///////////////////////////////
...
//////////////////////// 課題3. ///////////////////////////////
// 擬似コードの ① X = X U {u} の部分．
done[u] = true;// 編集: 課題3 doneのu成分をtrueに更新
...
//////////////////////// 課題3. ///////////////////////////////
      ...
      for (int v = 0; v < numV; v++) {
        if (!done[v] && adj[u][v] != 0 && distance[u] + adj[u][v] < distance[v]) {
...
//////////////////////// 課題4. ///////////////////////////////
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (!done[v] && distance[u] + list.get(i).weight < distance[v]) {
